...
void MTCNN::init_model(const string &proto_model_dir) {
...
//    ONet_->CopyTrainedLayersFrom(proto_model_dir + "/det3.caffemodel");
    PNet_.reset(new Net<float>((proto_model_dir + "/12net.prototxt"), TEST));
    PNet_->CopyTrainedLayersFrom(proto_model_dir + "/det1.caffemodel");
    RNet_.reset(new Net<float>((proto_model_dir + "/24net.prototxt"), TEST));
    RNet_->CopyTrainedLayersFrom(proto_model_dir + "/det2.caffemodel");
    ONet_.reset(new Net<float>((proto_model_dir + "/48net.prototxt"), TEST));
    ONet_->CopyTrainedLayersFrom(proto_model_dir + "/det3.caffemodel");
...
vector<FaceInfo>
MTCNN::NextStage(const cv::Mat &image, vector<FaceInfo> &pre_stage_res, int input_w, int input_h, int stage_num, const float threshold) {
    vector<FaceInfo> res;
